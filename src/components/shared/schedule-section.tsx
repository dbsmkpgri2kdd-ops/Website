  ...
  return (
    <section className="py-16 max-w-7xl mx-auto px-6 animate-fade-in">
      <div className="text-center mb-12">
        <h2 className="text-4xl font-bold font-headline text-primary">Jadwal Pelajaran</h2>
        <p className="text-lg text-muted-foreground mt-2">Lihat jadwal pelajaran untuk setiap kelas.</p>
      </div>

      <div className="max-w-md mx-auto mb-8">
        <Select onValueChange={setSelectedClass} value={selectedClass} disabled={isLoading || classNames.length === 0}>
            <SelectTrigger>
                <SelectValue placeholder={isLoading ? "Memuat kelas..." : "Pilih kelas untuk melihat jadwal"} />
            </SelectTrigger>
            <SelectContent>
                {classNames.map(name => (
          ...
          <CardContent className='p-6'>
            ...
            {!isLoading && selectedClass && (
             ...
             {!isLoading && selectedClass && Object.keys(filteredAndGroupedSchedule).length === 0 && (
                 <div className="text-center py-20 text-muted-foreground">
                    <p>Jadwal untuk kelas {selectedClass} belum ditambahkan oleh admin.</p>
                </div>
             )}
             {!isLoading && classNames.length === 0 && (
                 <div className="text-center py-20 text-muted-foreground">
                    <p>Admin belum menambahkan data jadwal pelajaran.</p>
                </div>
             )}
          </CardContent>
      </Card>
