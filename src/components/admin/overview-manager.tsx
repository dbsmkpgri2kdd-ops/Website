    ...
    <div className="space-y-8 animate-reveal pb-20">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="space-y-6">
            <Card className="border-primary/10 bg-card rounded-[2.5rem] shadow-sm overflow-hidden relative group h-full">
                <CardHeader className='p-8'>
                    <CardTitle className='text-sm font-bold flex items-center gap-2 text-foreground'>
                        <Sparkles size={16} className='text-primary' /> Analisis AI
                    </CardTitle>
                    <CardDescription className='text-xs font-medium text-muted-foreground'>Wawasan cerdas v1.0</CardDescription>
                </CardHeader>
                            ...
                            <div className='space-y-2'>
                                {aiReport.insights.map((insight, idx) => (
                                    <div key={idx} className='flex gap-2 items-start'>
                                        ...
                                        <p className='text-xs font-semibold text-muted-foreground leading-tight'>{insight}</p>
                                    </div>
                                ))}
                            </div>
                            <Button onClick={() => setAiReport(null)} variant="ghost" className="w-full text-[10px] font-bold text-muted-foreground hover:text-primary">Reset analisis</Button>
                        </div>
                    ) : (
                        <div className='text-center space-y-6 py-4'>
                            <p className='text-xs font-medium text-muted-foreground leading-relaxed'>Gunakan kecerdasan buatan untuk menganalisis tren pendaftaran dan aktivitas terbaru.</p>
                            <Button 
                  ...
                  <div key={app.id} className="flex items-center justify-between p-5 rounded-2xl hover:bg-muted transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-medium text-muted-foreground mt-0.5">{app.chosenMajor}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <Badge variant="secondary" className="text-[9px] font-bold mt-1.5 px-2.5 py-0.5 rounded-full bg-muted text-muted-foreground border-none">{app.status || 'Pending'}</Badge>
                    </div>
                  </div>
                )) : <div className="text-center py-16 opacity-40"><UserPlus size={40} className="mx-auto mb-3" /><p className="text-xs font-bold text-muted-foreground">Belum ada pendaftar baru</p></div>
              )}
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-8 space-y-6">
            {[
                ...
                <div key={idx} className="flex items-center gap-4 group">
                    ...
                    <div>
                        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">{infra.label}</p>
