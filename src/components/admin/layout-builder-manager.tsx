...
import { GripVertical, MoveUp, MoveDown, Save, Layout } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      <Alert className="bg-primary/5 border-primary/20">
        ...
        <AlertTitle className='font-bold text-xs'>Page Builder v2.0</AlertTitle>
        <AlertDescription className='text-xs font-medium'>
          Atur urutan bagian pada halaman depan dengan memindahkan posisi modul di bawah ini.
        </AlertDescription>
      </Alert>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
          ...
          {sections.map((section, idx) => {
            ...
            return (
              <Card key={section.id} className={cn(
                "shadow-sm border-slate-200 bg-white rounded-2xl hover:border-primary/20 transition-all group overflow-hidden",
                ...
                <CardContent className="p-0 flex items-center">
                  ...
                  <div className="flex-1 p-6">
                    <div className='flex items-center gap-3 mb-1'>
                      <h4 className="font-bold text-sm tracking-tight text-slate-900">{section.label}</h4>
                      {!isVisible && <div className='px-2 py-0.5 bg-slate-100 text-slate-500 text-[9px] font-bold rounded'>Nonaktif</div>}
                    </div>
                    <p className="text-[11px] text-muted-foreground font-medium leading-relaxed max-w-md">{section.desc}</p>
                  </div>
        ...
        <div className='space-y-6'>
          <Card className='rounded-2xl bg-primary/5 border-primary/10'>
            <CardHeader>
              <CardTitle className='text-sm font-bold'>Petunjuk penggunaan</CardTitle>
            </CardHeader>
            <CardContent className='text-xs space-y-4 text-muted-foreground font-medium leading-relaxed'>
              ...
              <p>2. Modul yang tampil pudar sedang dinonaktifkan melalui menu Konfigurasi.</p>
              <p>3. Klik Simpan Tata Letak untuk menerapkan perubahan secara publik.</p>
            </CardContent>
          </Card>
          
          <div className="sticky top-24">
            <Button onClick={handleSave} size="lg" className="w-full font-bold h-16 rounded-2xl shadow-xl shadow-primary/20 hover:scale-[1.02] transition-all">
