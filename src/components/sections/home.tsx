...
const HomeSection = ({ setActiveTab, onSelectArticle }: HomeSectionProps) => {
  ...
  return (
      <div className="animate-fade-in space-y-16 md:space-y-40 pb-20 overflow-x-hidden">
        <section className="relative min-h-[85vh] md:min-h-[90vh] -mt-20 flex items-center overflow-hidden bg-[#0a0c1b]">
          ...
          <Image
              ...
              priority
            />
          <div className="relative z-20 max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center pt-20 md:pt-0">
              <div className="space-y-6 md:space-y-10">
                <div className="space-y-3 md:space-y-4">
                  ...
                  <h1 className="text-4xl md:text-7xl font-black text-white font-headline leading-[1.1] tracking-tighter">
                ...
                <div className="flex flex-col sm:flex-row gap-4 md:gap-6">
                  <button onClick={() => setActiveTab('ppdb-online')} className="bg-primary text-white h-14 md:h-16 px-8 md:px-12 rounded-full font-black text-lg md:text-xl shadow-xl shadow-primary/20 hover:scale-105 transition-all duration-300 flex items-center justify-center gap-2">
                      Daftar PPDB <ArrowRight className='h-5 w-5 md:h-6 md:w-6' />
                  </button>
                  <button onClick={() => setActiveTab('profil-sejarah')} className="h-14 md:h-16 px-8 md:px-12 rounded-full font-bold text-lg md:text-xl text-white border border-white/20 hover:bg-white/10 backdrop-blur-sm transition-all">
                      Profil Sekolah
                  </button>
                </div>
              </div>
        ...
        {!arePartnersLoading && partners && partners.length > 0 && (
          <section className="-mt-20 md:-mt-32 relative z-30 max-w-7xl mx-auto px-4 md:px-6">
            ...
            <div className='space-y-6 md:space-y-8'>
              <div className="space-y-3 md:space-y-4">
               ...
               {isSchoolDataLoading ? (
                ...
                <div className="text-muted-foreground text-lg md:text-xl leading-relaxed font-medium">
                ...
                <div className="grid md:grid-cols-3 gap-6 md:gap-10">
                    {areMajorsLoading ? (
                      Array.from({ length: 3 }).map((_, i) => (
                        <Skeleton key={i} className="h-[300px] md:h-[400px] rounded-[2rem] md:rounded-[3rem]" />
                      ))
                    ) : (
                      majors?.slice(0, 3).map((major) => {
                        ...
                        return (
                             <Card key={major.id} className="p-8 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-sm hover:shadow-2xl bg-card border-none flex flex-col group overflow-hidden relative">
                                  <div className="w-14 h-14 md:w-20 md:h-20 bg-primary text-white rounded-2xl md:rounded-3xl flex items-center justify-center mb-6 md:submit-10">
                                      <Icon className="w-8 h-8 md:w-10 md:h-10" />
                                  </div>
                                  <h3 className="text-2xl md:text-3xl font-black font-headline mb-4 md:mb-6 group-hover:text-primary transition-colors tracking-tight leading-tight">{major.name}</h3>
                                  <p className="text-muted-foreground text-base md:text-lg leading-relaxed font-medium flex-grow mb-6 md:mb-8">{major.description}</p>
        ...
        <section className="py-20 md:py-32 bg-[#0a0c1b] text-white overflow-hidden relative">
            <div className="max-w-7xl mx-auto px-6 relative z-10">
                ...
                <div className="grid md:grid-cols-3 gap-8 md:gap-12">
                  {areNewsLoading ? (
                    Array.from({ length: 3 }).map((_, i) => (
                      <Skeleton key={i} className="h-[400px] md:h-[500px] rounded-[2rem] md:rounded-[3rem] bg-white/5" />
                    ))
                  ) : (
                    newsArticles?.map((news) => (
                      <Card key={news.id} className="rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-2xl group cursor-pointer border-none bg-white/[0.03] backdrop-blur-sm" onClick={() => onSelectArticle(news.id)}>
                        <div className="p-0 h-56 md:h-72 overflow-hidden relative">
                          <Image 
                        ...
                        <CardContent className="p-6 md:p-10 space-y-4 md:space-y-6">
                          ...
                          <h3 className="text-xl md:text-2xl font-black font-headline leading-tight group-hover:text-primary transition-colors line-clamp-2 tracking-tight">{news.title}</h3>
                        </CardContent>
                      </Card>
                    ))
                  )}
        ...
        <section className="max-w-7xl mx-auto px-4 md:px-6 pb-20">
          <div className="bg-[#0a0c1b] text-white rounded-[3rem] md:rounded-[4rem] p-10 md:p-32 text-center flex flex-col items-center relative overflow-hidden shadow-2xl">
            <h2 className="text-4xl md:text-7xl font-black font-headline mb-8 md:mb-10 leading-[1.1] tracking-tighter relative z-10">Siap Menjadi <br/> <span className='text-primary'>Ahli di Bidangnya?</span></h2>
